...
static stList *getRandomGappedPinchSet(stPinchThreadSet *threadSet) {
    ...
    int64_t threadEnd1 = stPinchThread_getStart(thread1) + stPinchThread_getLength(thread1);
    int64_t prevEnd2 = stPinchThread_getStart(thread2);
    int64_t threadEnd2 = stPinchThread_getStart(thread2) + stPinchThread_getLength(thread2);

    if (thread1 == thread2) {
        // prevent overlaps!
        if (stPinchThread_getLength(thread1) == 1) {
            // No possible non-overlapping pinches.
            return ret;
        }
        prevEnd2 = st_randomInt64(prevEnd1 + 1, threadEnd2);
        threadEnd1 = prevEnd2;
    }
    for (int64_t i = 0; i < maxNumPinches; i++) {
        int64_t start1 = st_randomInt(prevEnd1, threadEnd1);
        int64_t start2 = st_randomInt(prevEnd2, threadEnd2);
        int64_t maxLength1 = threadEnd1 - start1;
        int64_t maxLength2 = threadEnd2 - start2;
            ...
            if (st_random() > 0.5) {
                ...
                for (int64_t i = 0; i < stList_length(pinches); i++) {
                    ...
                    for (int64_t j = 0; j < pinch->length; j++) {
                        mergePositionsSymmetric(columns, pinch->name1, pinch->start1 + j, 1, pinch->name2,
                                                pinch->strand ? pinch->start2 + j : pinch->start2 + pinch->length - 1 - j, pinch->strand);
                    }
                }
            } else {
                // Find all the pinched blocks and undo them.
                stPinchSegment *segment = stPinchThread_getFirst(thread1);
                while (segment != NULL) {
                    stPinchBlock *block = stPinchSegment_getBlock(segment);
                    if (block != NULL) {
                ...
                segment = stPinchThread_getFirst(thread2);
                while (segment != NULL) {
                    stPinchBlock *block = stPinchSegment_getBlock(segment);
                    if (block != NULL) {
                        int64_t undoOffset, undoLength;
